...
class DifferentiationVisitor(NodeVisitor):
    """ Визитор для вычисления производной. Обходит AST и применяет правила дифференцирования """
    ...
    def visit_BinaryOpNode(self, node: BinaryOpNode) -> ASTNode:
        """ Дифференцирование бинарных операций """
        ...
        # степень (для константного показателя)
        elif node.operator == '^':
            if isinstance(node.right, NumberNode):
                ...
                raise Exception(f"Степень с переменным показателем пока не поддерживается: {node.left}^{node.right}")

        return NumberNode(0)

    def visit_FunctionNode(self, node: FunctionNode) -> ASTNode:
        """ Правила дифференцирования для функций """
